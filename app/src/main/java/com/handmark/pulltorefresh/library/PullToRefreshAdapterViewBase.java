...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements
    ...
    public void scrollBy(final int x, final int y) {
        mhandler.postDelayed(new Runnable() {
            @Override
            public void run() {
                ((AdapterView<ListAdapter>) mRefreshableView).scrollBy(x, y);
                System.out.println("---class" + mRefreshableView.getClass().toString());
            }
        }, 3000);
    }

    public void smoothScrollBy(int y,int time) {
        ((GridView) mRefreshableView).smoothScrollBy(y,time);
    }
