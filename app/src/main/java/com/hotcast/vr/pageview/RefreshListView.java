...
public class RefreshListView extends ListView implements OnScrollListener {
	...
	private View ll_refreshheard;

	public final int DOWN_PULL_REFRESH = 0; // 下拉刷新状态
	public final int RELEASE_REFRESH = 1; // 松开刷新
	public final int REFRESHING = 2; // 正在刷新中
	...
	private void initHeaderView() {
		...
		ll_refreshheard = headerView.findViewById(R.id.ll_refreshheard);
		vp_itempager = (ViewPager) headerView.findViewById(R.id.vp_itempager);
		...
		ll_refreshheard.measure(0, 0);
		headerViewHeight = ll_refreshheard.getMeasuredHeight();
		headerView.setPadding(0, -headerViewHeight, 0, 0);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			int m = 0;
			topY = vp_itempager.getTop();
			View view = (View) vp_itempager.getParent();
			while (m<=1) {
				topY = view.getTop()+topY;
		...
		case MotionEvent.ACTION_MOVE:
			...
			topY = 0;
			topY = vp_itempager.getTop();
			View view1 = (View) vp_itempager.getParent();
			while (m1<=1) {
				topY = view1.getTop()+topY;
			...
			int paddingTop = -headerViewHeight+topY+ diff;
			// 如果: -头布局的高度 > paddingTop的值 执行super.onTouchEvent(ev);

			if (firstVisibleItemPosition == 0 && -headerViewHeight < paddingTop) {
				if (paddingTop -headerViewHeight> 0 && currentState == DOWN_PULL_REFRESH) { // 完全显示了.
					Log.i(TAG, "松开刷新");
					currentState = RELEASE_REFRESH;
					refreshHeaderView();
				} else if (paddingTop-headerViewHeight< 0 && currentState == RELEASE_REFRESH) { // 没有显示完全
					Log.i(TAG, "下拉刷新");
	...
	public ViewPager getViewPager() {
		...
		return vp_itempager = (ViewPager) headerView
				.findViewById(R.id.vp_itempager);

	}

	public TextView getTvImme() {
		return tv_Imme = (TextView) headerView.findViewById(R.id.tv_Imme);

	}

	public TextView getTvImmeInfo() {
		return tvImmeInfo = (TextView) headerView.findViewById(R.id.tvImmeInfo);

	}

	public TextView getTvshare() {
		return tv_share = (TextView) headerView.findViewById(R.id.tv_share);
